                    ...
                    <CarouselContent>
                        {reviews.map((review: ReviewProps, index: number) => (
                            <CarouselItem key={index} className="md:basis-1/2lg:basis-1/3 relative">
                                <div className="absolute inset-0 w-4/5 h-20 top-1/4 left-1/2 -translate-x-1/2 -rotate-45 lg:h-52 bg-pink-500/10 blur-[8rem]"></div>
    ...
    const renderStars = (rating: number) => {
        ...
        for (let i = 0; i < 5; i++) {
            if (i < rating) {
                ...
                stars.push(<StarIcon key={i} className="size-4 lg:size-5 text-foreground/20" />);
            }
        }
        return stars;
    };
